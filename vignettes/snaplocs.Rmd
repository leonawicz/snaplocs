...
An additional `region` argument can be passed to any of these functions to protect against this possibility.
`region` may be a vector.

```{r helpers4}
r <- "Alaska"
get_region(x, region = r)
get_country(x, r)
get_coords(x, r)
get_coords(x, r, keep_cols = TRUE)
```
